...
- (void)setupToolbars
{
    ...
    for (int i=0; i<self.btnArr.count; i++) {
        // 创建按钮
        NSString *title = self.btnArr[i];
        UIButton *otherBtn = [[UIButton alloc] init];
        [otherBtn setTitle:self.btnArr[i] forState:UIControlStateNormal];
        [otherBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        otherBtn.layer.borderWidth = 0.1;
        otherBtn.layer.borderColor = [UIColor whiteColor].CGColor;
        otherBtn.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.3f];
        otherBtn.layer.cornerRadius = 2;
        otherBtn.clipsToBounds = YES;
        otherBtn.tag = 13500+i;
        [otherBtn addTarget:self action:@selector(otherBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        _otherButton = otherBtn;
        CGSize buttonSize = [title getSizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake(MAXFLOAT, 15)];
        otherBtn.frame = CGRectMake(x, self.bounds.size.height-70, buttonSize.width+20, 30);
        x+=otherBtn.frame.size.width+10;
...
-(void)otherBtnClick:(UIButton *)button{
    if (self.otherBtnBlock){
        self.otherBtnBlock(button.tag-13500);
    }
}
...
- (void)saveImage
{
    ...
    if (currentView.hasLoadedImage) {
        ...
        if ([currentView.imageview isKindOfClass:[FLAnimatedImageView class]]) {
            ...
            //FIXME:1:修改为保存到指定相册功能，如果不设置相册，则自由保存
//            PHPhotoLibrary
            // 1. 获取相片库对象
            ...
            [library performChanges:^{
                ...
                // 2.2 取出指定名称的相册
                PHAssetCollection *assetCollection = [self getCurrentPhotoCollectionWithTitle:@"pornhub"];
                // 2.3 判断相册是否存在
